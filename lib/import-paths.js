...
async function writeFileChanges(fileName, matches, tsImports) {
  ...
  fileContents = fileContents
    // Ensure there is at most one blank line between each statement.
    .replace(/\n{3,}/g, '\n\n')
    ...
    .replace(/;import/g, ';\nimport')
    // Fix instances where an import statement ends up on the same line as another statement.
    .replace(/(import[\w\W]+?from\s*[\w\W]+?['"]\s*;)([^\s])/g, '$1\n\n$2')
